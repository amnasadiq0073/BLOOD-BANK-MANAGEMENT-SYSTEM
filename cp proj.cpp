...
const char* staffname1 = "Sameen";
const char* staffname2 = "Faiza";
const char* staffname3 = "Amna";
const char* staffname4 = "Kisa";
/********************************** Date class***************************************/
class Date{
	int date;
	int month ;
...
void User::assign_un(){
	...
	gotoxy (45,17); cout << "THANK YOU FOR CHOOSING SAFK BLOOD BANK";
	getch();	
	user_home(s);
}
...
void draw_symbol(int x, int y, int t){
	...
	for (i = 1; i <= 2; i++){
		switch(i){
			case 1:
				gotoxy(x+5, y+2); printStringSlowly("SAFK", t);
				break;
			case 2:
...
void mainmenu_general(){
	...
	printLineSlowly('^', 40, 5);
	gotoxy(80,12); 
	printStringSlowly("WELCOME TO SAFK BLOOD BANK", 5);
	gotoxy(106,12); 
	printLineSlowly('^', 40, 5);
	draw_symbol(130, 15, 5);
	gotoxy(57,18); cout<<"                            ";
   	...
   	if (str == "ENTER PASSWORD: "){
   		...
   		if(strcmp(pass,staffname1)==0 || strcmp(pass,staffname2)==0 || strcmp(pass,staffname3)==0){
    		...
    		return true;
     		...
     		gotoxy(45,16);
     		cout<<"INVALID USERNAME. If You are not an Staff member Please Press Backspace.";
     		ch = getch();
     		if (ch == 8){
     			mainmenu_general();
			}
